...
CoatedDiffuseBxDFMixedParticles CoatedDiffuseMaterialMixedParticles::GetBxDF(TextureEvaluator texEval,
    ...
    // Cosmetic map
    // We can use the cosmetic map to change the model parameters in this way, we could create only the skin case where the cosmetic map is not 1.0
    Float cosmetic_map_value = texEval(cosmetic_map, ctx);

    if (cosmetic_map_value < 0.8){

        //std::cout << "Cosmetic map value : " << cosmetic_map_value << std::endl;
...
CoatedDiffuseMaterialMixedParticles *CoatedDiffuseMaterialMixedParticles::Create(
    ...
    FloatTexture cosmetic_map = parameters.GetFloatTextureOrNull("cosmetic_map", alloc);
    
    //std::cout << "Cosmetic map value : " << cosmetic_map << std::endl;
    ...
    return alloc.new_object<CoatedDiffuseMaterialMixedParticles>(
        ...
        normalMap, remapRoughness, maxDepth, nSamples, cosmetic_map);
}
