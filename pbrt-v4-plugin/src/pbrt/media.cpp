...
// SGGX Method Definitions
std::string SGGXPhaseFunction::ToString() const {
    return StringPrintf("[ SGGXPhaseFunction sigma: %f ]", sigma);
}
...
HomogeneousMediumSGGX *HomogeneousMediumSGGX::Create(const ParameterDictionary &parameters,
                                             const FileLoc *loc, Allocator alloc) {
    Spectrum sig_a = nullptr, sig_s = nullptr;
    std::string preset = parameters.GetOneString("preset", "");
    if (!preset.empty()) {
    ...
    Float sigmaScale = parameters.GetOneFloat("scale", 1.f);
    Float g = parameters.GetOneFloat("g", 0.0f);    
    Float x = parameters.GetOneFloat("x", 0.0f);    
    Float y = parameters.GetOneFloat("y", 0.0f);    
    Float z = parameters.GetOneFloat("z", 1.0f);    
    Vector3f dir(x,y,z);
    return alloc.new_object<HomogeneousMediumSGGX>(sig_a, sig_s, sigmaScale, Le, LeScale, g,dir,
                                               alloc);
}

std::string HomogeneousMediumSGGX::ToString() const {
    return StringPrintf(
        "[ Homogeneous medium sigma_a_spec: %s sigma_s_spec: %s Le_spec: %s phase: %s ]",
        sigma_a_spec, sigma_s_spec, Le_spec, phase);
}
...
// SGGXMedium Method Definitions ----------------------------------------------------
SGGXMedium::SGGXMedium(const Transform &renderFromMedium, Spectrum sigma_a,
                             Spectrum sigma_s, Float sigmaScale, Float g,
                             nanovdb::GridHandle<NanoVDBBuffer> dg,
                             nanovdb::GridHandle<NanoVDBBuffer> tg, Float LeScale,
                             Float temperatureOffset, Float temperatureScale,
...
std::string SGGXMedium::ToString() const {
    return StringPrintf("[ SGGXMedium bounds: %s LeScale: %f "
                        "temperatureOffset: %f temperatureScale: %f (grids elided) ]",
                        bounds, LeScale, temperatureOffset, temperatureScale);
}

SGGXMedium *SGGXMedium::Create(const ParameterDictionary &parameters,
                                     const Transform &renderFromMedium,
                                     const FileLoc *loc, Allocator alloc) {
    std::string filename = ResolveFilename(parameters.GetOneString("filename", ""));
    if (filename.empty())
    ...
    return alloc.new_object<SGGXMedium>(
        renderFromMedium, sigma_a, sigma_s, sigmaScale, g, std::move(densityGrid),
        std::move(temperatureGrid), LeScale, temperatureOffset, temperatureScale, alloc);
}
...
Medium Medium::Create(const std::string &name, const ParameterDictionary &parameters,
    ...
    if (name == "homogeneous")
        ...
        m = SGGXMedium::Create(parameters, renderFromMedium, loc, alloc);
    } else if (name == "sggxhomogeneous") {
        m = HomogeneousMediumSGGX::Create(parameters, loc, alloc);
    } else
