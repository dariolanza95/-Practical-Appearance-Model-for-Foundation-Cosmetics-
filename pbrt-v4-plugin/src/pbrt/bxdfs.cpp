...
//DL utility function
size_t getIndex(Float value,Float minRange,Float maxRange,int resolution){
    if (value < minRange || value > maxRange) {
        return -1; // Value is out of range
    }
    auto binSize = (maxRange - minRange) / resolution;
    int binIndex = std::floor((value - minRange) / binSize);
    binIndex = std::min(resolution, std::max(binIndex,0) );
    return (size_t)binIndex;
}
...
template <typename TopBxDF, typename BottomBxDF, bool twoSided>
std::string MyLayeredBxDF<TopBxDF, BottomBxDF, twoSided>::ToString() const {
    return StringPrintf(
        "[ MyLayeredBxDF top: %s bottom: %s thickness: %f albedo: %s g: %f ]", top, bottom,
        thickness, albedo, g);
}

template <typename TopBxDF, typename BottomBxDF, bool twoSided>
std::string LayeredBxDFMixedParticles<TopBxDF, BottomBxDF, twoSided>::ToString() const {
    return StringPrintf(
        //TODO - remove this        
        "[ LayeredBxDFMixedParticles top: %s bottom: %s thickness: %f albedo: %s g: %f ]", top, bottom,
        thickness, albedo_diff,albedo_flakes, g1,g2,w_g);
}
template <typename TopBxDF, typename BottomBxDF, bool twoSided>
std::string LayeredBxDFMixedParticlesTransmittance<TopBxDF, BottomBxDF, twoSided>::ToString() const {
    return StringPrintf(
        //TODO - remove this        
        "[ LayeredBxDFMixedParticlesTransmittance top: %s bottom: %s thickness: %f albedo: %s g: %f ]", top, bottom,
        thickness, albedo_diff,albedo_flakes, g1,g2,w_g);
}
template <typename TopBxDF, typename BottomBxDF, bool twoSided>
std::string LayeredBxDFMixedParticlesTransmittanceAdvanced<TopBxDF, BottomBxDF, twoSided>::ToString() const {
    return StringPrintf(
        //TODO - remove this        
        "[ LayeredBxDFMixedParticlesTransmittanceAdvanced top: %s bottom: %s thickness: %f albedo: %s g: %f ]", top, bottom,
        media1.thickness, media1.albedo_diff,media1.albedo_flakes, media1.g1,media1.g2,media1.w_g);
}

template <typename TopBxDF, typename BottomBxDF, bool twoSided>
std::string LayeredBxDFMixedParticlesNormalizedFresnel<TopBxDF, BottomBxDF, twoSided>::ToString() const {
    return StringPrintf(
        //TODO - remove this        
        "[ LayeredBxDFMixedParticlesNormalizedFresnel top: %s bottom: %s thickness: %f albedo: %s g: %f ]", top, bottom,
        thickness, albedo_diff,albedo_flakes, g1,g2,w_g);
}

template <typename TopBxDF, typename BottomBxDF, bool twoSided>
std::string LayeredBxDFMixedParticlesNormalizedFresnelAdvanced<TopBxDF, BottomBxDF, twoSided>::ToString() const {
    return StringPrintf(
        //TODO - remove this        
        "[ LayeredBxDFMixedParticlesNormalizedFresnelAdvanced top: %s bottom: %s thickness: %f albedo: %s g: %f ]", top, bottom,
        media1.thickness, media1.albedo_diff,media1.albedo_flakes, media1.g1,media1.g2,media1.w_g);
}
...
#define SAFE_READ(vars, size, count) \
    ...
    for (uint32_t i = 0; i < n_fields; ++i) {
        ...
        std::cout<<"name "<< name << " has type "<<  " with offset "<< static_cast<size_t>(offset) << " shape " << ndim << " and starts at " << std::endl;
        m_fields[name] =
...
// MyMeasuredBxDFData Definition
struct MyMeasuredBxDFData {
    // MyMeasuredBxDFData Public Members
    pstd::vector<float> wavelengths;
    // std::vector<std::vector<std::vector<Float>> > m_grod(phi_o_res, std::vector<int>(columns, 0));
    ...
    MyMeasuredBxDFData(Allocator alloc)
        : ndf(alloc),
          sigma(alloc),
          vndf(alloc),
          luminance(alloc),
    ...
    static MyMeasuredBxDFData *Create(const std::string &filename, Allocator alloc);

    std::string ToString() const {
        return StringPrintf("[ MyMeasuredBxDFData filename: %s ]", filename);
    }
...
MyMeasuredBxDFData *MyMeasuredBxDFData::Create(const std::string &filename, Allocator alloc) {
    ...
    std::ifstream inputFile("example.txt"); // Replace "example.txt" with the file path
    ...
    while (header >> component) {
            components.push_back(component);
    }
    // int theta_i_res = intcomponents[0],theta_o_res = components[1],wl_res = components[2];
    int theta_i_res = 90,theta_o_res = 90,wl_res = 80;
    std::vector<Float> m_grid(theta_i_res*theta_o_res*wl_res);

    int index = 0;
    //we expect the file to be already sorted in the following order wavelenghts|theta_i|theta_0
    while (std::getline(inputFile, line)) {
        index += 1;
        m_grid[index] = std::stof(line);
    }
    ...
    // Float val(0.0f);
    // for (int theta_o_idx = 0;theta_o_idx<theta_o_res;theta_o_idx++){
    //     val = 0;
    //     for (int theta_i_idx = 0;theta_i_idx<theta_i_res;theta_i_idx++){
    //         for (int wl_idx= 0;wl_idx<wl_res;wl_idx++){
    //             size_t full_idx =  theta_o_idx * theta_i_res * wl_res +  theta_i_idx * wl_res + wl_idx;
    //             // std::cout<<"full_idx "<< full_idx << std::endl;
    //             m_grid[full_idx] = 0.5/Pi;
    //             }
    //         val += 1;
    // }}
    ...
    MyMeasuredBxDFData *brdf = alloc.new_object<MyMeasuredBxDFData>(alloc);
    ...
    measuredBRDFBytes += sizeof(MyMeasuredBxDFData) + 4 * brdf->wavelengths.size() +
...
MyMeasuredBxDFData *MyMeasuredBxDF::BRDFDataFromFile(const std::string &filename,
                                                 Allocator alloc) {
    static std::map<std::string, MyMeasuredBxDFData *> loadedData;
    if (loadedData.find(filename) == loadedData.end())
        loadedData[filename] = MyMeasuredBxDFData::Create(filename, alloc);
    return loadedData[filename];
}

// MyMeasuredBxDF Method Definitions
SampledSpectrum MyMeasuredBxDF::f(Vector3f wo, Vector3f wi, TransportMode mode) const {
    ...
    // Float rand_val = brdf->m_grid[0];
    // Float rand_val2 = brdf->m_grid[647999];
    // Float theta_o = 30;
    // Float theta_i = 30;

    
    // wl_idx = getIndex(wl,360,780,theta_i_res);
    
    // std::cout<<"rand val" << rand_val2<< std::endl;
    // Evaluate inverse parameterization $R^{-1}$
    PLSample ui = brdf->vndf.Invert(u_wm, phi_o, theta_o);
*/
    Float theta_o = SphericalTheta(wo), phi_o = std::atan2(wo.y, wo.x);
    Float theta_i = SphericalTheta(wo), phi_i = std::atan2(wi.y, wi.x);

    //Evaluate spectral 5D interpolant
    SampledSpectrum fr;
    // for (int i = 0; i < NSpectrumSamples; ++i)
        // fr[i] = std::max<Float>(0, brdf->spectra.Evaluate(ui.p, phi_o, theta_o, lambda[i]));
    size_t theta_o_idx = getIndex(theta_o *(180/Pi) ,0,90,theta_o_res);
    size_t theta_i_idx = getIndex(theta_i *(180/Pi) ,0,90,theta_i_res);
    for (int i = 0; i < NSpectrumSamples; ++i){
        size_t wl_idx = getIndex(lambda[i],360,780,wl_res);
        size_t full_idx =  theta_o_idx * theta_i_res * wl_res +  theta_i_idx * wl_res + wl_idx;
        fr[i] = brdf->m_grid[full_idx];
        // fr[i] = std::max<Float>(0, brdf->spectra.Evaluate(ui.p, phi_o, theta_o, lambda[i]));
    }
    // std::cout<<"fr is "<< fr << std::endl;
    return fr;//* CosTheta(wi);
    ...
    // for (int i = 0; i < NSpectrumSamples; ++i)
        //  fr[i] = std::max<Float>(0, brdf->spectra.Evaluate(Point2f(0.0,0.0), phi_o, theta_o, lambda[i]));
    // return fr* CosTheta(wi);
}

pstd::optional<BSDFSample> MyMeasuredBxDF::Sample_f(Vector3f wo, Float uc, Point2f u,
                                                  TransportMode mode,
                                                  BxDFReflTransFlags sampleFlags) const {
    ...
    */
    ...
    // Float theta_i = 30;
    // size_t theta_o_idx = getIndex(theta_o,0,90,theta_o_res);
    // size_t theta_i_idx = getIndex(theta_i,0,90,theta_i_res);
    size_t theta_i_idx = getIndex(theta_i *(180/Pi) ,0,90,theta_i_res);
    size_t theta_o_idx = getIndex(theta_o *(180/Pi) ,0,90,theta_i_res);
    ...
    // return BSDFSample(fr* AbsCosTheta(wi), wi, pdf , BxDFFlags::DiffuseReflection);
    return BSDFSample(fr, wi, pdf , BxDFFlags::DiffuseReflection);
}

Float MyMeasuredBxDF::PDF(Vector3f wo, Vector3f wi, TransportMode mode,
                        BxDFReflTransFlags sampleFlags) const {
...
std::string MyMeasuredBxDF::ToString() const {
    return StringPrintf("[ MyMeasuredBxDF brdf: %s ]", *brdf);
...
STAT_MEMORY_COUNTER("Memory/Measured BRDF data", MymeasuredBRDFBytes);
...
MeasuredBxDFData *MeasuredBxDFData::Create(const std::string &filename, Allocator alloc) {
    ...
    MymeasuredBRDFBytes += sizeof(MyMeasuredBxDFData) + 4 * brdf->wavelengths.size() +
                         brdf->ndf.BytesUsed() + brdf->sigma.BytesUsed() +
                         brdf->vndf.BytesUsed() + brdf->luminance.BytesUsed() +
                         brdf->spectra.BytesUsed();
...
template class MyLayeredBxDF<DielectricBxDF, DiffuseBxDF, true>;
template class LayeredBxDFMixedParticles<DielectricBxDF, DiffuseBxDF, true>;

template class LayeredBxDFMixedParticlesTransmittance<DielectricBxDF, DielectricBxDF, false>;
template class LayeredBxDFMixedParticlesTransmittanceAdvanced<DielectricBxDF, DielectricBxDF, false>;
template class LayeredBxDFMixedParticlesNormalizedFresnel<DielectricBxDF,NormalizedFresnelBxDF , false>;
template class LayeredBxDFMixedParticlesNormalizedFresnelAdvanced<DielectricBxDF,NormalizedFresnelBxDF , false>;

// template class LayeredBxDFMixedParticles<DielectricBxDF, DielectricBxDF, true>;
...
SampledSpectrum SheenVolumeBxDF::f(Vector3f wo, Vector3f wi, TransportMode mode) const{
    Float cosThetaO = CosTheta(wo),
          cosThetaI = CosTheta(wi);
    if (maxBounces == 0 || cosThetaO < 0 || cosThetaI < 0) return SampledSpectrum(0.f);

    /*
    printf("maxBounces = %d, sigma = %f, density = %f\n", maxBounces, sigma, density);
    ...
    */
    ...
    SampledSpectrum beta(1.f);  // Path throughput
    SampledSpectrum value(0.f); // Accumulator for BRDF * cos
    ...
    while (true) {
        ...
        if (CosTheta(wi) > 0.0) {
            ...
            Float sigmaTAlongWi = density*sggx::projArea(-wi, S);
        ...
        // Scattering event.
        beta *= albedo;
        dir = sggx::samplePhaseSpecular(-dir, S, rng.Uniform<Float>(), rng.Uniform<Float>());
    ...
    return cosThetaI > 0.f ? value / cosThetaI : SampledSpectrum(0.f);
}

pstd::optional<BSDFSample> SheenVolumeBxDF::Sample_f(Vector3f wo, Float uc, Point2f u, TransportMode mode, BxDFReflTransFlags 
    sampleFlags) const {
    /* Naive uniform hemisphere sampling. At least this is guaranteed to result
       in a reflection direction. */
    Vector3f wi = SampleUniformHemisphere(u);
    Float pdf = 0.f;    
    SampledSpectrum res = SampledSpectrum(0.f);

    if (CosTheta(wo) < 0.f || CosTheta(wi) < 0.f) {

        return BSDFSample(res, wi, pdf, BxDFFlags::DiffuseReflection);
    }
    
    //res = f(wo, wi, mode);
    res = SampledSpectrum(1.f);
    pdf = PDF(wo, wi, mode, sampleFlags);

    /* This value is actually never used and is instead recomputed in `integrator.cpp`. So let's avoid another costly random walk here. */
    // return f(wo, *wi);
    // return 1.f;
...
Float SheenVolumeBxDF::PDF(const Vector3f wo, const Vector3f wi, TransportMode mode, BxDFReflTransFlags sampleFlags) const {
    if (CosTheta(wo) < 0.f || CosTheta(wi) < 0.f) {
        return 0.f;
    }
...
std::string SheenVolumeBxDF::ToString() const {
