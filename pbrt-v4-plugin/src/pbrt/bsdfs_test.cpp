...
/*
BSDF* createLayeredMaterial(const SurfaceInteraction& si, Allocator alloc) {
    Float defaultRefl(0.05);
    Float defaultRoughness(0.05);
    Float defaultThickness(0.05);
    Float defaultEta(1.5);
    bool use_specular = true;
    Float defaultDisplacement(0.0f);
    Float defaultAlbedo(0.99);
    int maxDepth = 5; 
    int nSamples = 5;
    Float sigma(0.1);    
    Float mean(45.0);
    ...
    Vector3f dir(0,0,1);
    SpectrumTexture reflectance = alloc.new_object<SpectrumConstantTexture>(alloc.new_object<ConstantSpectrum>(defaultRefl));
    

    FloatTexture uRoughness = alloc.new_object<FloatConstantTexture>(defaultRoughness);
    FloatTexture vRoughness = alloc.new_object<FloatConstantTexture>(defaultRoughness);


    FloatTexture thickness = alloc.new_object<FloatConstantTexture>(defaultThickness);

    Spectrum eta = alloc.new_object<ConstantSpectrum>(defaultEta);  

    FloatTexture g = alloc.new_object<FloatConstantTexture>(defaultG);
    
    SpectrumTexture albedo = alloc.new_object<SpectrumConstantTexture>(alloc.new_object<ConstantSpectrum>(defaultAlbedo));

    FloatTexture displacement = alloc.new_object<FloatConstantTexture>(defaultDisplacement);
    
    

    return alloc.new_object<MyCoatedDiffuseMaterial>(
        reflectance, uRoughness, vRoughness, thickness,dir,sigma,mean,std,use_specular, albedo, g, eta, displacement,
