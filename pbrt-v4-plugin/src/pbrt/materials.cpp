...
template <typename TextureEvaluator>
MyCoatedDiffuseBxDF MyCoatedDiffuseMaterial::GetBxDF(TextureEvaluator texEval,
                                                 const MaterialEvalContext &ctx,
                                                 SampledWavelengths &lambda) const {
    // Initialize diffuse component of plastic material
    SampledSpectrum r = Clamp(texEval(reflectance, ctx, lambda), 0, 1);
    ...
    return MyCoatedDiffuseBxDF(DielectricBxDF(sampledEta, distrib), DiffuseBxDF(r), thick, dir,sigma,mean,std_or,std_size,use_refl,
                             a, gg, maxDepth, nSamples);
}
// Explicit template instantiation
template MyCoatedDiffuseBxDF MyCoatedDiffuseMaterial::GetBxDF(
    BasicTextureEvaluator, const MaterialEvalContext &ctx,
    SampledWavelengths &lambda) const;
...
template <typename TextureEvaluator>
CoatedDiffuseBxDFMixedParticles CoatedDiffuseMaterialMixedParticles::GetBxDF(TextureEvaluator texEval,
                                                 const MaterialEvalContext &ctx,
                                                 SampledWavelengths &lambda) const {
    // Initialize diffuse component of plastic material
    SampledSpectrum r = Clamp(texEval(reflectance, ctx, lambda), 0, 1);
    ...
    return CoatedDiffuseBxDFMixedParticles(DielectricBxDF(sampledEta, distrib), DiffuseBxDF(r), thick, dir,sigma,mean,std_or,std_size,diff_particles_perc,use_refl,
                             a_flake,a_diff, g1_c,g2_c,w_g_c, maxDepth, nSamples);
}
// Explicit template instantiation
template CoatedDiffuseBxDFMixedParticles CoatedDiffuseMaterialMixedParticles::GetBxDF(
    BasicTextureEvaluator, const MaterialEvalContext &ctx,
    SampledWavelengths &lambda) const;
template CoatedDiffuseBxDFMixedParticles CoatedDiffuseMaterialMixedParticles::GetBxDF(
    UniversalTextureEvaluator, const MaterialEvalContext &ctx,
...
template <typename TextureEvaluator>
TransmittanceBxDFMixedParticles TransmittanceMaterialMixedParticles::GetBxDF(TextureEvaluator texEval,
                                                 const MaterialEvalContext &ctx,
                                                 SampledWavelengths &lambda) const {
    // Initialize diffuse component of plastic material
    SampledSpectrum r = Clamp(texEval(reflectance, ctx, lambda), 0, 1);
    ...
    Float g2_c = Clamp(texEval(g2, ctx), -1, 1);
    Float w_g_c = Clamp(texEval(w_g, ctx), -1, 1);
    Float sampledEtaTop = 1.0f;
    TrowbridgeReitzDistribution distribTop(Float(0.0f), Float(0.0f));
    std::cout<<"l409 platelets_roughness "<< platelets_roughness << std::endl;
    return TransmittanceBxDFMixedParticles(DielectricBxDF(sampledEtaTop, distribTop), DielectricBxDF(sampledEta, distrib), thick, dir,platelets_roughness,mean,std_or,std_size,diff_particles_perc,use_refl,
                             a_flake,a_diff, g1_c,g2_c,w_g_c, maxDepth, nSamples);
}
// Explicit template instantiation
template TransmittanceBxDFMixedParticles TransmittanceMaterialMixedParticles::GetBxDF(
    BasicTextureEvaluator, const MaterialEvalContext &ctx,
    SampledWavelengths &lambda) const;
template TransmittanceBxDFMixedParticles TransmittanceMaterialMixedParticles::GetBxDF(
    UniversalTextureEvaluator, const MaterialEvalContext &ctx,
...
CoatedDiffuseMaterial *CoatedDiffuseMaterial::Create(
    ...
    SpectrumTexture albedo = parameters.GetSpectrumTexture("albedo", nullptr, SpectrumType::Albedo, alloc);
    if (!albedo)
...
std::string MyCoatedDiffuseMaterial::ToString() const {
    return StringPrintf(
        "[ MyCoatedDiffuseMaterial displacement: %s normalMap: %s reflectance: %s "
        "uRoughness: %s vRoughness: %s thickness: %s eta: %s remapRoughness: %s ]",
        displacement, normalMap ? normalMap->ToString() : std::string("(nullptr)"),
        reflectance, uRoughness, vRoughness, thickness, eta, remapRoughness);
}

MyCoatedDiffuseMaterial *MyCoatedDiffuseMaterial::Create(
    const TextureParameterDictionary &parameters, Image *normalMap, const FileLoc *loc,
    Allocator alloc) {
    SpectrumTexture reflectance = parameters.GetSpectrumTexture(
        "reflectance", nullptr, SpectrumType::Albedo, alloc);
    ...
    Float x = parameters.GetOneFloat("x",0);
    Float y = parameters.GetOneFloat("y",0);
    Float z = parameters.GetOneFloat("z",1);
    Float mean = parameters.GetOneFloat("mean",0.0f);
    Float std_or = parameters.GetOneFloat("std_or",0.0f);
    Float std_size = parameters.GetOneFloat("stdSize",0.0f);
    
    bool use_specular = parameters.GetOneBool("use_specular", true);
    Float sigma = parameters.GetOneFloat("sigma",0.1);
    ...
    Vector3f dir(x,y,z);
    return alloc.new_object<MyCoatedDiffuseMaterial>(
        reflectance, uRoughness, vRoughness, thickness,dir,sigma,mean,std_or,std_size,use_specular, albedo, g, eta, displacement,
        normalMap, remapRoughness, maxDepth, nSamples);
}


std::string CoatedDiffuseMaterialMixedParticles::ToString() const {
    return StringPrintf(
        "[ MyCoatedDiffuseMaterial displacement: %s normalMap: %s reflectance: %s "
        "uRoughness: %s vRoughness: %s thickness: %s eta: %s remapRoughness: %s ]",
        displacement, normalMap ? normalMap->ToString() : std::string("(nullptr)"),
...
CoatedDiffuseMaterialMixedParticles *CoatedDiffuseMaterialMixedParticles::Create(
    const TextureParameterDictionary &parameters, Image *normalMap, const FileLoc *loc,
    Allocator alloc) {
    SpectrumTexture reflectance = parameters.GetSpectrumTexture(
        "reflectance", nullptr, SpectrumType::Albedo, alloc);
    ...
    Float std_or = parameters.GetOneFloat("std_or",0.0f);
    Float std_size = parameters.GetOneFloat("stdSize",0.0f);
    Float diff_particles_perc = parameters.GetOneFloat("diff_particles_perc",0.0f); 
    bool use_specular = parameters.GetOneBool("use_specular", true);
    Float sigma = parameters.GetOneFloat("sigma",0.1);
    // Float g = parameters.GetOneFloat("g",1);

    FloatTexture g1 = parameters.GetFloatTexture("g1", 0.f, alloc);
    FloatTexture g2 = parameters.GetFloatTexture("g2", 0.f, alloc);
    FloatTexture w_g = parameters.GetFloatTexture("w_g", 0.f, alloc);
    SpectrumTexture albedo_flakes = parameters.GetSpectrumTexture("albedo_flakes", nullptr, SpectrumType::Albedo, alloc);
    if (!albedo_flakes)
        albedo_flakes = alloc.new_object<SpectrumConstantTexture>(
            alloc.new_object<ConstantSpectrum>(0.f));
    SpectrumTexture albedo_diff = parameters.GetSpectrumTexture("albedo_diff", nullptr, SpectrumType::Albedo, alloc);
    if (!albedo_diff)
        albedo_diff = alloc.new_object<SpectrumConstantTexture>(
            alloc.new_object<ConstantSpectrum>(0.f));
    ...
    Vector3f dir(x,y,z);
    return alloc.new_object<CoatedDiffuseMaterialMixedParticles>(
        reflectance, uRoughness, vRoughness, thickness,dir,sigma,mean,std_or,std_size,diff_particles_perc,use_specular, albedo_flakes,albedo_diff, g1,g2,w_g, eta, displacement,
        normalMap, remapRoughness, maxDepth, nSamples);
}

std::string TransmittanceMaterialMixedParticles::ToString() const {
    return StringPrintf(
        "[ MyCoatedDiffuseMaterial displacement: %s normalMap: %s reflectance: %s "
        "uRoughness: %s vRoughness: %s thickness: %s eta: %s remapRoughness: %s ]",
        displacement, normalMap ? normalMap->ToString() : std::string("(nullptr)"),
...
TransmittanceMaterialMixedParticles *TransmittanceMaterialMixedParticles::Create(
    const TextureParameterDictionary &parameters, Image *normalMap, const FileLoc *loc,
    Allocator alloc) {
    SpectrumTexture reflectance = parameters.GetSpectrumTexture(
        "reflectance", nullptr, SpectrumType::Albedo, alloc);
    ...
    Vector3f dir(x,y,z);
    return alloc.new_object<TransmittanceMaterialMixedParticles>(
        reflectance, uRoughness, vRoughness, thickness,dir,sigma,mean,std_or,std_size,diff_particles_perc,use_specular, albedo_flakes,albedo_diff, g1,g2,w_g, eta, displacement,
        normalMap, remapRoughness, maxDepth, nSamples);
}
...
// SubsurfaceMaterialCosmetic Method Definitions
std::string SubsurfaceMaterialCosmetic::ToString() const {
    return StringPrintf("[ SubsurfaceMaterialCosmetic displacement: %s normalMap: %s scale: %f "
                        "sigma_a: %s sigma_s: %s reflectance: %s mfp: %s uRoughness: %s "
                        "vRoughness: %s scale: %f eta: %f remapRoughness: %s ]",
                        displacement, normalMap, scale, sigma_a, sigma_s, reflectance, mfp,
                        uRoughness, vRoughness, scale, etaSkin, remapRoughness);
}

SubsurfaceMaterialCosmetic *SubsurfaceMaterialCosmetic::Create(
    const TextureParameterDictionary &parameters, Image *normalMap, const FileLoc *loc,
    Allocator alloc) {
    SpectrumTexture sigma_a, sigma_s, reflectance, mfp;
    ...
    Float scale = parameters.GetOneFloat("scale", 1.f);
    Float etaSkin = parameters.GetOneFloat("etaSkin", 1.33f);
    ...
    Float std_or = parameters.GetOneFloat("std_or",0.0f);
    Float std_size = parameters.GetOneFloat("stdSize",0.0f);
    Float thickness_scale = parameters.GetOneFloat("thick_scale",1.0f);
    Float mean_p_scale = parameters.GetOneFloat("mean_p_scale",1.0f);
    ...
    Spectrum eta2;
    if (!parameters.GetFloatArray("eta2").empty())
        eta2 = alloc.new_object<ConstantSpectrum>(parameters.GetFloatArray("eta2")[0]);
    else
        eta2 = parameters.GetOneSpectrum("eta", nullptr, SpectrumType::Unbounded, alloc);
    if (!eta2)
        eta2 = alloc.new_object<ConstantSpectrum>(1.5f);

    Vector3f dir(x,y,z);
    FloatTexture thickness = parameters.GetFloatTexture("thickness", .0, alloc);
    FloatTexture mean_p_orientation = parameters.GetFloatTexture("mean_p_orientation", .0, alloc);
    return alloc.new_object<SubsurfaceMaterialCosmetic>(
        scale, sigma_a, sigma_s, reflectance, mfp, g, etaSkin, uRoughness, vRoughness,thickness,dir,sigma,mean_p_orientation,std_or,std_size,diff_particles_perc,use_specular, albedo_flakes,albedo_diff, g1,g2,w_g, eta2,thickness_scale,mean_p_scale, displacement, 
        normalMap, remapRoughness, maxDepth, nSamples, alloc);

}


std::string SubsurfaceMaterialCosmeticAdvanced::ToString() const {
    return StringPrintf("[ SubsurfaceMaterialCosmeticAdvanced displacement: %s normalMap: %s scale: %f "
                        "sigma_a: %s sigma_s: %s reflectance: %s mfp: %s uRoughness: %s "
                        "vRoughness: %s scale: %f eta: %f remapRoughness: %s ]",
                        displacement, normalMap, scale, sigma_a, sigma_s, reflectance, mfp,
                        uRoughness, vRoughness, scale, etaSkin, remapRoughness);
...
SubsurfaceMaterialCosmeticAdvanced *SubsurfaceMaterialCosmeticAdvanced::Create(
    const TextureParameterDictionary &parameters, Image *normalMap, const FileLoc *loc,
    Allocator alloc) {
    SpectrumTexture sigma_a, sigma_s, reflectance, mfp;
    ...
    //Top Layer
    FloatTexture g1_top = parameters.GetFloatTexture("g1_top", 0.f, alloc);
    FloatTexture g2_top = parameters.GetFloatTexture("g2_top", 0.f, alloc);
    FloatTexture w_g_top = parameters.GetFloatTexture("w_g_top", 0.f, alloc);
    SpectrumTexture albedo_flakes_top = parameters.GetSpectrumTexture("albedo_flakes_top", nullptr, SpectrumType::Albedo, alloc);
    if (!albedo_flakes_top)
        albedo_flakes_top = alloc.new_object<SpectrumConstantTexture>(
            alloc.new_object<ConstantSpectrum>(0.f));
    SpectrumTexture albedo_diff_top = parameters.GetSpectrumTexture("albedo_diff_top", nullptr, SpectrumType::Albedo, alloc);
    if (!albedo_diff_top)
        albedo_diff_top = alloc.new_object<SpectrumConstantTexture>(
            alloc.new_object<ConstantSpectrum>(0.f));


        Float std_or_top = parameters.GetOneFloat("std_or_top",0.0f);
    Float std_size_top = parameters.GetOneFloat("stdSize_top",0.0f);
    Float thickness_scale_top = parameters.GetOneFloat("thick_scale_top",1.0f);
    Float mean_p_scale_top = parameters.GetOneFloat("mean_p_scale_top",1.0f);
    

    Float diff_particles_perc_top = parameters.GetOneFloat("diff_particles_perc_top",0.0f); 
    Float sigma_top = parameters.GetOneFloat("sigma_top",0.1);
    // Float g = parameters.GetOneFloat("g",1);
    FloatTexture thickness_top = parameters.GetFloatTexture("thickness_top", .0, alloc);
    FloatTexture mean_p_orientation_top = parameters.GetFloatTexture("mean_p_orientation_top", .0, alloc);
...
CosmeticStructMaterial media_mat2(albedo_flakes_top, albedo_diff_top, thickness_top,
     g1_top,
     g2_top,
     w_g_top,
     mean_p_orientation_top,
     thickness_scale_top,
     mean_p_scale_top,
     sigma_top,
     std_or_top,
     std_size_top,
     diff_particles_perc_top );
CosmeticStructMaterial media_mat1(albedo_flakes, albedo_diff, thickness,
     g1,
     g2,
     w_g,
     mean_p_orientation,
     ...
     std_size,
     diff_particles_perc );
return alloc.new_object<SubsurfaceMaterialCosmeticAdvanced>(
    scale, sigma_a, sigma_s,media_mat1, media_mat2, reflectance, mfp, g, etaSkin, uRoughness, vRoughness, use_specular, eta2, displacement,normalMap, remapRoughness, maxDepth, nSamples, alloc);
...
// MyMeasuredMaterial 
MyMeasuredMaterial::MyMeasuredMaterial(const std::string &filename, FloatTexture displacement,
                                   Image *normalMap, Allocator alloc)
    : displacement(displacement), normalMap(normalMap) {
    brdf = MyMeasuredBxDF::BRDFDataFromFile(filename, alloc);
}

std::string MyMeasuredMaterial::ToString() const {
    return StringPrintf("[ MyMeasuredMaterial displacement: %s normalMap: %s ]",
                        displacement,
                        normalMap ? normalMap->ToString() : std::string("(nullptr)"));
}

MyMeasuredMaterial *MyMeasuredMaterial::Create(const TextureParameterDictionary &parameters,
                                           Image *normalMap, const FileLoc *loc,
                                           Allocator alloc) {
    std::string filename = ResolveFilename(parameters.GetOneString("filename", ""));
    if (filename.empty()) {
        Error("Filename must be provided for MyMeasuredMaterial");
        return nullptr;
    }
    FloatTexture displacement = parameters.GetFloatTextureOrNull("displacement", alloc);

    return alloc.new_object<MyMeasuredMaterial>(filename, displacement, normalMap, alloc);
}


// New material!
SheenVolumeMaterial *SheenVolumeMaterial::Create(const TextureParameterDictionary &parameters,
                                           Image *normalMap, const FileLoc *loc,
                                           Allocator alloc) {
    int maxBounces = parameters.GetOneInt("maxBounces", 16);
    SpectrumTexture albedo = parameters.GetSpectrumTexture("albedo", nullptr, SpectrumType::Albedo, alloc);

    if (!albedo){
        albedo = alloc.new_object<SpectrumConstantTexture>(alloc.new_object<ConstantSpectrum>(1.f));
    }

    FloatTexture density = parameters.GetFloatTexture("density", 1.f, alloc);
    FloatTexture sigma = parameters.GetFloatTexture("sigma", 0.5f, alloc);
    FloatTexture bumpMap = parameters.GetFloatTextureOrNull("bumpmap", alloc);

    return alloc.new_object<SheenVolumeMaterial>(maxBounces, albedo, density, sigma, bumpMap);
}

std::string SheenVolumeMaterial::ToString() const {
    return "";
}
...
Material Material::Create(const std::string &name,
    ...
    if (name.empty() || name == "none") {
        ...
        material = MyCoatedDiffuseMaterial::Create(parameters, normalMap, loc, alloc);
    else if (name == "mybsdfMixed")
        material = CoatedDiffuseMaterialMixedParticles::Create(parameters, normalMap, loc, alloc);
    else if (name == "myCosmetic")
        material = TransmittanceMaterialMixedParticles::Create(parameters, normalMap, loc, alloc);
    ...
    else if (name == "MyMeasured")
        material = MyMeasuredMaterial::Create(parameters, normalMap, loc, alloc);
    else if (name == "subsurface")
        material = SubsurfaceMaterial::Create(parameters, normalMap, loc, alloc);
    else if (name == "subsurfaceCosmetic")
        material = SubsurfaceMaterialCosmetic::Create(parameters, normalMap, loc, alloc);
    else if (name == "subsurfaceCosmeticAdvanced")
        material = SubsurfaceMaterialCosmeticAdvanced::Create(parameters, normalMap, loc, alloc);
    else if (name == "sheenvolume")
        material = SheenVolumeMaterial::Create(parameters, normalMap, loc, alloc);
    else if (name == "mix") {
