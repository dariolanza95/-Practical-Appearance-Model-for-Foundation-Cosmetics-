...
def getBSDFData(basename, helper, out_folder,output_file="",data_file="",backrefl_data = False):
    ...
    helper.verbose = True
    data_test = helper.loadData(data_file, output_file )
    ...
    if output_file.endswith(".npy"):
        ...
        if data.shape != (m,n,3):
            print("WARNING!")
            err_msg = "Please provide a file with shape {0}x{1}x3, where m angle of incidence and n angle of observations x 3 rgb channels.".format(m,n)
            print(err_msg)
            print(data.shape)
            breakpoint()
    else:
...
def plotPatch(ax,img,isErrorData = False,use_ticks = True):
    ...
    #print(np.mean(img))
    if use_ticks:
        custom_y_ticks = [0, 1, 2, 3,4]
        custom_y_tick_labels = ['30', '40', '50', '60','70']
...
def objective(x, data_values, parameters, parameter_names, query):
    ...
    # Ignore highlight (focus on diffuse) --> Replace by 0s
    # Theshold would probably depend on measurements
...
def main():
    ...
    parser.add_argument("-isHQ", action="store_true", help="Set this flag to have HQ results with one iteration only (for final results.)")
